...
# calculate uppr and lower limits of the CI of 95%
lower_limit <- mean_difference - 1.96*sd(df_CHR_BA$difference )
upper_limit <- mean_difference + 1.96*sd(df_CHR_BA$difference )
...
# calculate uppr and lower limits of the CI of 95%
lower_limit <- mean_difference - 1.96*sd(df_disease_BA$difference )
upper_limit <- mean_difference + 1.96*sd(df_disease_BA$difference )
...
ggplot(df_disease_BA, aes(x = average, y = difference)) +
  ...
  geom_hline(yintercept = upper_limit, color = "#009E73", lwd=2) +
  geom_point(shape=23, fill="#56B4E9", size=6) +
  annotate("text", x = max(df_disease_BA$average), y = min(df_disease_BA$difference),
